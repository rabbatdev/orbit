...
Built on [`@orbit/orm-core`](../orm-core), so the same machinery can back other ORMs later; Drizzle is the first adapter.
...
```ts
import type { GenerateOptions } from '@orbit/drizzle/cli';
...
import { defineOrbitSchema } from '@orbit/drizzle';
...
```ts
import { createBuilder } from '@orbit/client';
...
Implement `OrmAdapter` from `@orbit/orm-core` — i.e. a single `toIR()` that produces a
